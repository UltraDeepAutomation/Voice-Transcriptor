...
| BUG-46 | P2 | ⚠️ принятый риск: изоляция окружений движка — отдельное архитектурное решение (вопрос владельцу) |
...
Итог дня: волна 1 — 15/15 исправлено; волна 2 — **13/14 исправлено**,
1 (BUG-46) вынесен на решение об изоляции окружений движка.
Верификация волны 2: backend 247 тестов OK (+5 регрессионных), frontend
tsc + ESLint чисто + 60 тестов vitest, desktop node --test 15 pass,
`node --check` main.js.

## Вопрос к владельцу продукта (не блокирует остальные фиксы)

1. **BUG-46 / архитектура движка:** оставить PYTHONPATH-shadowing как
   принятый риск или развести окружения (отдельный venv для GigaAM)?
2. **ENABLE_GIGAAM в пакете:** маркер копируется в каждый DMG/инсталлятор
   (`desktop/package.json`: files + extraResources), поэтому авто-установка
   ~6 GB движка запускается у каждого пользователя при первом запуске
   (после фикса BUG-39 — только при наличии сети). Если это было dev-only
   переключение, убрать его из `files`/`extraResources`; если осознанная
   поставка «движок по требованию» — оставить как есть.
